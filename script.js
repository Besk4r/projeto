...
document.addEventListener('DOMContentLoaded', function() {
    ...
    menu.addEventListener("click", (e) => 
        { let parentButton = e.target.closest(".add-to-cart-btn"); 
            if (parentButton) { addToCart(parentButton.getAttribute("data-name"), parseFloat(parentButton.getAttribute("data-price"))); } 
        });
    ...
    { cartItemsContainer.innerHTML = ""; 
        let subtotal = 0; let totalItems = 0; cart.forEach 
        (item => 
            { 
                const 
                ...
                <p class="font-bold">${item.name}</p><p>Qtd: ${item.quantity}</p>
                <p class="font-medium mt-2">R$ ${item.price.toFixed(2)}</p></div><button 
                class="remove-from-cart-btn text-red-500" data-name="${item.name}">Remover</button></div>`; 
                subtotal += item.price * item.quantity; totalItems += item.quantity; 
    ...
    (o => o.addEventListener
        ...
        calculateShippingBtn.addEventListener
    (
        "click", () => 
        { const cep = cepInput.value; if(!cep) return; const cepPrefix = cep.substring(0, 5); if(shippingRates[cepPrefix] !== undefined) 
            { shippingRate = 
            ...
            { Toastify(
                ...
                if(payment === "pix")
                    { const totalPedido = cart.reduce((total, item) => total + (item.price * item.quantity), 0) + shippingRate; 
                        ...
                        cartModal.classList.add("hidden"); pixModal.classList.remove("hidden"); 
                        pixModal.classList.add("flex"); return; 
                    } sendOrderToWhatsApp(); 
        }
    );
    ...
    { 
        ...
        const isOpen = checkRestaurantStatus(); if(isOpen)
    { 
    ...
    (
        ...
        (
            "click", () => 
            { 
                infoModal.classList.add("hidden"); infoModal.classList.remove("flex"); 
            }
        );
        ...
        (
            "click", (e) => 
            { 
                if(e.target === infoModal) { infoModal.classList.add("hidden"); infoModal.classList.remove("flex"); } 
            }
        );
        infoTabs.forEach
        (
    ...
    // --- LÓGICA DO MODAL DE LOGIN ADM ---
        ...
        (
            ...
            adminCloseModalBtn.addEventListener
        (
            "click", () => 
            { 
            ...
            "click", () => 
                ...
                { 
                    Toastify({ text: "Login efetuado com sucesso!", duration: 3000, gravity: "top", position: "right", style: { background: "#16a34a" } }).showToast(); 
                        ...
                        logoutContainer.classList.remove("hidden"); enterAdminMode(); 
                        adminLoginModal.classList.add("hidden"); adminLoginModal.classList.remove("flex"); 
                } 
                    else 
                { Toastify(
                    { text: "Usuário ou senha inválidos!", 
                        duration: 3000, gravity: "top", 
                        position: "right", style: { background: "#ef4444" } 
                    }).showToast(); 
                } document.getElementById("admin-user-input").value = ""; document.getElementById("admin-password-input").value = ""; });
    ...
    { 
        ...
        { 
            const addSobremesaBtnHTML = 
            `<button id="add-sobremesa-btn" class="w-full bg-green-500 text-white py-2 rounded-lg my-4 font-bold">+ Adicionar Nova Sobremesa</button>`; 
        ...
        (   
            ...
            { if (card.querySelector(".edit-item-btn")) return; if (card.children[1]) 
                { 
                    ...
                    class="edit-item-btn bg-blue-500 text-white px-3 py-1 rounded text-sm">Editar</button><button 
                    class="delete-item-btn bg-red-500 text-white px-3 py-1 rounded text-sm">Excluir</button></div>`; 
