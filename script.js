...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- FUNÇÕES PARA SALVAR E CARREGAR O ESTADO DO PEDIDO ---
    ...
    function populateMenu() {
        ...
        menuData.forEach(item => {
            ...
            card.innerHTML = `<img src="${item.image}" alt="${item.name}" 
                class="w-28 h-28 rounded-md hover:scale-110 hover:-rotate-2 duration-300"/><div>
                <p class="font-bold">${item.name}</p><p class="text-sm">${item.description}</p><div class="flex items-center gap-2 justify-between mt-3">
                <p class="font-bold text-lg">R$ ${item.price.toFixed(2).replace('.', ',')}</p>
                    <button class="bg-gray-900 px-5 rounded add-to-cart-btn" 
                        data-name="${item.name}" 
                        data-price="${item.price}"><i class="fa fa-cart-plus text-lg text-white"></i></button></div></div>`;
    ...
    paymentOptions.forEach(o => o.addEventListener("change", (e) => {
    ...
    // MODIFICADO: Reseta o frete ao digitar novo CEP para forçar recálculo
    cepInput.addEventListener("input", () => {
        shippingFeeText.innerHTML = ""; // Limpa a mensagem de frete
        shippingRate = 0;             // Zera a taxa de frete
        updateCartModal();            // Atualiza o total no carrinho
        saveOrderState();             // Salva o estado atualizado (com frete zerado)
    });

    // --- LÓGICA DE FINALIZAÇÃO DO PEDIDO ---
    checkoutBtn.addEventListener("click", () => {
        // Descomente a linha abaixo para reativar a verificação de horário
        // if (!checkRestaurantStatus()) {
        //     Toastify({ text: "DESCULPE, O RESTAURANTE ESTÁ FECHADO!", duration: 3000, gravity: "top", position: "right", style: { background: "#ef4444" } }).showToast();
        //     return;
        // }
        ...
        // ADICIONADO: Valida se o frete para entrega foi calculado e é válido
        if (delivery === "delivery" && shippingRate === 0) {
            Toastify({ text: "Por favor, informe e calcule um CEP válido para a entrega.", duration: 3000, gravity: "top", position: "right", style: { background: "#ef4444" } }).showToast();
        ...
        if (payment === "pix") {
            ...
            closeCartModal(); // MODIFICADO: Fecha o modal do carrinho com animação
    ...
    copyPixBtn.addEventListener("click", () => {
        ...
        Toastify({ text: "Chave PIX copiada!", duration: 2000, gravity: "top", position: "right", style: { background: "#16a34a" } }).showToast(); 
    });
    ...
    function sendOrderToWhatsApp(paymentFromPix = null) {
        ...
        // Esconde os modais sem animação, pois a página já estará redirecionando
    ...
    function checkRestaurantStatus() {
        return true; // Mantido desativado conforme solicitado
    ...
    loginBtn.addEventListener("click", () => { 
        ...
        if (user === userInput && pass === passInput) { 
            Toastify({ text: "Login efetuado com sucesso!", duration: 3000, gravity: "top", position: "right", style: { background: "#16a34a" } }).showToast(); 
            ...
            Toastify({ text: "Usuário ou senha inválidos!", duration: 3000, gravity: "top", position: "right", style: { background: "#ef4444" } }).showToast(); 
        }
    ...
    function enterAdminMode() { 
        if (!document.getElementById("add-lanche-btn")) { 
            const addLancheBtnHTML = `<button id="add-lanche-btn" class="w-full bg-green-500 text-white py-2 rounded-lg my-4 font-bold">+ Adicionar Novo Lanche</button>`; 
            menuLanches.insertAdjacentHTML('afterend', addLancheBtnHTML); 
        } 
        if (!document.getElementById("add-bebida-btn")) { 
            const addBebidaBtnHTML = `<button id="add-bebida-btn" class="w-full bg-green-500 text-white py-2 rounded-lg my-4 font-bold">+ Adicionar Nova Bebida</button>`; 
            menuBebidas.insertAdjacentHTML('afterend', addBebidaBtnHTML); 
        } 
        if (!document.getElementById("add-sobremesa-btn")) { 
            const addSobremesaBtnHTML = `<button id="add-sobremesa-btn" class="w-full bg-green-500 text-white py-2 rounded-lg my-4 font-bold">+ Adicionar Nova Sobremesa</button>`; 
            menuSobremesas.insertAdjacentHTML('afterend', addSobremesaBtnHTML); 
        } 
        const productCards = document.querySelectorAll(".product-card"); 
        productCards.forEach(card => {
            if (card.querySelector(".edit-item-btn")) return;
            if (card.children[1]) { 
                const adminButtonsHTML = `
                    <div class="flex gap-2 mt-2">
                        <button class="edit-item-btn bg-blue-500 text-white px-3 py-1 rounded text-sm">Editar</button>
                        <button class="delete-item-btn bg-red-500 text-white px-3 py-1 rounded text-sm">Excluir</button>
                    </div>`; 
                card.children[1].insertAdjacentHTML('beforeend', adminButtonsHTML); 
            } 
        }); 
    }
